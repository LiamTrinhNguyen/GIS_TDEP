...
2. data/input/site_variable_coverage.json

Each year string is always length 52 (week 1 → week 52).
'1' = has data that week, '0' = missing.
"""
...
def process_network(csv_path: Path, network_name: str):
    ...
    # Year + week-of-year (1–53, we will clamp to 1–52)
    df = df.with_columns([
    ...
    # ----- 2. Coverage: always 52 weeks per year -----
    coverage = {}
    ...
    for site_id, site_df in df.group_by("SITE_ID"):
        ...
        for var in VARIABLES:
            ...
            for year, year_df in site_df.group_by("year"):
                ...
                # Start with 52 zeros (week 1 → week 52)
                flags = ["0"] * 52
                ...
                for row in year_df.iter_rows(named=True):
                    ...
                    if week is None:
                        continue
                    # Clamp to 1–52 (ISO week 53 → treat as 52)
                    week_idx = min(int(week), 52) - 1   # 0-based index

                    # Mark as present if the variable has a value
                    val = row[var]
                    if val is not None:
                        flags[week_idx] = "1"
